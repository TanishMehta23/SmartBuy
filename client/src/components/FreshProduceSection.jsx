  ...
  return (
    <section ref={ref} className="my-6 sm:my-10">
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-emerald-950/90 via-teal-950/80 to-slate-900/95 text-white p-5 sm:p-7 border border-emerald-500/20 shadow-xl">
        ...
        {/* Section Top Header */}
        <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-3.5 sm:gap-4 mb-5 sm:mb-6">
          {/* Title & Badge Group (Always Full Width Single Line on Mobile) */}
          <div className="min-w-0 flex-1">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] sm:text-xs font-bold bg-emerald-500/20 text-emerald-300 border border-emerald-400/30 mb-1.5 whitespace-nowrap">
              <Leaf className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
              <span>{isPortuguese ? 'Colheita Fresca Diária' : 'Daily Farm Fresh Harvest'}</span>
            </div>
            <h2 className="text-lg sm:text-2xl lg:text-3xl font-black tracking-tight text-white whitespace-nowrap truncate">
              {t('freshProduceTitle')}
            </h2>
            <p className="text-xs sm:text-sm text-emerald-100/80 font-medium mt-0.5 truncate block">
              {t('freshProduceSubtitle')}
            </p>
          </div>

          {/* Subcategory Switcher + Controls Row (Single Non-Wrapping Line) */}
          <div className="flex items-center justify-between gap-2 pt-2 md:pt-0 border-t md:border-t-0 border-emerald-500/20 w-full overflow-x-auto no-scrollbar">
            {/* Filter Switcher */}
            <div className="inline-flex p-0.5 sm:p-1 rounded-2xl bg-white/10 backdrop-blur-md border border-white/15 shrink-0">
              <button
                type="button"
                onClick={() => setActiveTab('both')}
                className={`px-2 sm:px-3 py-1 rounded-xl text-[11px] sm:text-xs font-bold transition-all cursor-pointer whitespace-nowrap ${
                  activeTab === 'both'
            ...
            <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
              {/* Carousel Arrows (Hidden on very narrow mobile, visible sm+) */}
              <div className="hidden xs:flex items-center gap-1">
                <button
                  type="button"
                  onClick={() => handleScroll('left')}
                  className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-white/15 hover:bg-white/25 border border-white/20 flex items-center justify-center text-white transition-all cursor-pointer active:scale-95 shadow-xs"
              ...
              <button
                ...
                className="inline-flex items-center gap-1 sm:gap-1.5 px-2.5 py-1 sm:px-3.5 sm:py-1.5 rounded-xl sm:rounded-2xl text-[11px] sm:text-xs font-bold bg-white text-emerald-950 hover:bg-emerald-50 transition-all cursor-pointer group shrink-0 shadow-md whitespace-nowrap"
              >
                <span className="hidden sm:inline">{t('exploreProduce')}</span>
                <span className="sm:hidden">{t('seeAll') || 'See all'}</span>
